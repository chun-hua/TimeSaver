...
自动追踪每一次专注会话，提供回顾与自我认知闭环。

**记录内容**：
- 会话基本信息：任务名、实际时长 vs 计划时长、日期时间
- 陪伴者与背景主题
- 状态分类：已完成 / 提前结束 / 已放弃（左侧色条：绿 / 琥珀 / 灰）
- 打断详情：每次退出的原因文本 + 最终选择（返回继续 / 冷静后离开 / 紧急退出）

**双屏历史系统**：

| 屏幕 | 内容 |
|------|------|
| **专注记录** | 顶部 3 卡片摘要（今日专注 / 会话数 / 完成率）→ 筛选胶囊（全部 / 已完成 / 有打断）→ 时间线卡片列表 |
| **本周统计** | 堆叠柱状图（紫=专注时长 / 琥珀=打断次数）→ 趋势对比卡片（vs 上周 / 连续专注天数 / 平均时长）→ 打断原因 Top N → 最佳时段推荐 |

**设计参考**：
- Linear 的暗色层级系统 + 左侧色条状态编码 + 筛选 Pill 组件
- Notion 的温暖卡片氛围 + Metric Cards 摘要模式
- Superhuman 的克制配色（仅紫/琥珀/翠绿 3 种强调色）

**技术实现**：
- 数据通过 `localStorage` 持久化，关闭浏览器不丢失（原型）
- Android 端对应 `Room` 数据库 `Session` Entity，DAO 查询按天/周聚合
- 键盘快捷键 `H` 循环切换：创建 → 历史 → 统计 → 创建
...
- 新增「专注记录」屏幕（`s-history`）：顶部 3 卡片摘要（今日专注 / 会话数 / 完成率）、3 项筛选胶囊（全部 / 已完成 / 有打断）、时间线卡片列表，每张卡片含左侧色条（绿=完成 / 琥珀=打断 / 灰=放弃）、任务图标、陪伴者、主题、时长、打断详情。
- 新增「本周统计」屏幕（`s-stats`）：周堆叠柱状图（紫=专注 / 琥珀=打断）、趋势对比（vs 上周 / 连续天数 / 平均时长）、打断原因 Top N（智能 emoji 匹配）、最佳时段推荐。
- 会话数据自动记录：计时完成 → 已完成；退出→写原因→冷静→返回 → 打断-返回；退出→冷静→离开 → 打断-离开；紧急退出 → 已放弃。
- 数据通过 `localStorage` 持久化（原型），Android 端对应 Room `Session` Entity。
- 键盘快捷键 `H` 循环切换：创建 → 历史 → 统计 → 创建。
- 设计语言融合 Linear（暗色层级+色条编码）、Notion（温暖卡片+Metric Cards）、Superhuman（克制三色配色）。
- 新增文件：`design/线框-历史记录.html`（Figma 级信息架构原型含设计标注）。
- 修改文件：`挽留者 原型.html`（+80 行 HTML）、`wanliu/styles.css`（+220 行 CSS）、`wanliu/app.js`（+400 行 JS）。

### v0603b — 2026-06-02

**9 位大师 8 幕粒子叙事全面升级**

- 为全部 9 位非叙事大师（费曼、塔勒布、马斯克、纳瓦尔、Paul Graham、爱因斯坦、达·芬奇、张一鸣、孙宇晨）设计完整的 8 幕粒子叙事（金句 + 章节 + 色调 + 上下文），每人 8 幕共 72 幕，保存为 `design/*_8stages.csv`。
- 生成全部 72 个粒子形态定义（几何原语数组），每位大师 8 个形态，独立色调映射（`design/masters_forms_complete.js`，70 KB）。
- 符号设计：费曼（问号→费曼图→O型环→螺旋）、塔勒布（黑天鹅翼→九头蛇→杠铃→沙漏）、马斯克（原子→爆炸X→堆叠塔→火箭）、纳瓦尔（天平→大脑→指南针→树）、PG（钢笔→阶梯→钻石→桥）、爱因斯坦（光束→原子→引力透镜→E=mc²）、达·芬奇（眼睛→维特鲁威→黄金螺旋→蒙娜丽莎弧）、张一鸣（网络节点→沙漏→盾牌→数据流）、孙宇晨（扩音器→火焰→皇冠→流星）。
- **app.js 5 处编辑**：① `C_FORMS` 从旧 3-form 周期替换为 11 组 8-stage builder；② 移除未使用 `SYM_` 常量；③ `_getStageTint()` 查表化支持全部 11 人；④ `nextForm()` 的 `stageMeta` 查表化；⑤ `updateStageDots()` 动态匹配陪伴者阶段数（8 点 vs 3 点）。
- HTML 引入 `masters_forms_complete.js`，所有陪伴者进入专注空间后自动启动 8 幕叙事循环。

**设计规范**

- 中文金句 ≤30 字，`\n` 两行、完整成句不截断；英文原文独立字段；上下文标注年份+事件。
- 每幕独立色调，相邻阶段平滑过渡；几何符号无面孔/肖像，全部程序化原语。
- 叙事弧完整：初心（散点云团）→ 发展 → 巅峰 → 坠落 → 转折 → 终极 → 开放结尾。
